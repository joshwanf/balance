...
type Req = Partial<ApiTypes.Transaction.ChangeTransactionRequest>
type Resp = ApiTypes.Transaction.Transaction
type Handler = ApiTypes.CustomRouteHandler<Req, Resp>
  ...
  try {
    ...
    const result = await pc.$transaction(async prisma => {
      ...
      if (itemName) {
        ...
        const itemCleanedName = cleanName(itemName)
        const item = await prisma.category.upsert({
          create: {
            ...
            userId: user.id,
            amount: transactionBody.amount || 0,
            // UserGroup: {
            //   create: {
            //     userId: user.id,
            //   },
